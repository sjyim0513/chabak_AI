...
class OpenAIGpt:
    ...
    def __init__(self):
        ...
        self.loc = set(["제주", "경기", "충청", "강원", "경상", "전라", "서울", "부산", "대구", "인천", "광주", "대전", "울산", "세종", "시", "봄", "여름", "가을", "겨울", "별", "바다", "바닷가", "가족", "반려", "커플", "힐링", "남친", "여친", "애인", "남자친구", "여자친구", "부>모님", "엄마", "아빠", "형", "누나", "동생"])
        #사용할 형태소 분석 모델
    ...
    def run(self, question):
        ...
        for loc in self.loc:
            if loc in prompt:
                ...
                for ent in doc.ents:
                    if ent.text in ['애인', '여치', '남친', '남자친구', '여자친구']:
                        url += ent.label_ + "=커플&"
                        continue
                    if ent.text in ["부모", "엄마", "아빠", "형", "누나", "동생"]:
                        url += ent.label_ + "=가족&"
                        continue
                    if ent.text in ["야영장", "캠핑장", "야영지", "캠핑지", "야영", "캠핑"]:
                        continue
                    if ent.text == "바닷가":
                        url += ent.label_ + "=바다&"
                        continue
        ...
        response = openai.Completion.create(
            engine="davinci:ft-personal-2023-06-04-23-58-41",
            prompt=prompt,
            temperature=0.3,
            max_tokens=256,
            top_p=1,
